...
# ---------------------------------------------------------
# 1. DEFINE PROXY MODEL (The Rule-Based Logic)
# ---------------------------------------------------------
# ---------------------------------------------------------
# 1. DEFINE PROXY MODEL (The HYBRID Logic)
# ---------------------------------------------------------
import joblib
# Load model globally to simulate backend
...
# ---------------------------------------------------------
# 2. CREATE GROUND TRUTH DATASET (Synthetic Data)
# ---------------------------------------------------------
# We create a dataset that has some "easy" cases and some "hard" cases to simulate a real evaluation.
dataset = [
    # Emergency
    ...
    ("Running a temperature of 102", "fever"), # "temperature" not in rules -> MIGHT FAIL (Good for discussion)
    
    # Headache
    ("Severe migraine", "headache"),
    ("My head hurts a lot", "headache"), # "head" + "pain" (split words) -> MIGHT FAIL
    ...
    # Common / Ambiguous (The "False Negatives")
    ("My knee hurts", "common"), # No rule for knee
    ("I feel tired", "common"),
    ("General weakness", "common"),
    ("Back pain", "common"), # No rule for back pain
    ("I cut my finger", "emergency"), # "bleeding" might catch it if user said bleeding, but here just "cut" -> FAIL
]
...
plt.ylabel('Actual')
plt.title('Confusion Matrix - Rule-Based System')
plt.tight_layout()
plt.savefig('confusion_matrix.png')
print("\n[SUCCESS] Generated 'confusion_matrix.png'")

# Error Analysis (Print mismatches)
print("\n[MISCLASSIFIED CASES - Use for 'Discussion' section]")
